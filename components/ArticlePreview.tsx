...
export const ArticlePreview: React.FC<ArticlePreviewProps> = ({ articles, onReset, onSave, onDelete }) => {
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {articles.map((article, idx) => (
              <button
                ...
                className={`w-full text-left p-3 rounded-lg text-sm transition-all border ${selectedArticleId === article.id
              ...
              >
                <div className="flex items-start">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    {selectedArticleId === article.id && <ChevronRight className="w-4 h-4 text-blue-500 ml-1" />}
                  </div>
                </div>
              </button>
            ))}
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 bg-white">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white shrink-0">
          ...
          <div className="flex items-center space-x-2 shrink-0">
            ...
            <button
              onClick={handleSave}
              className={`flex items-center px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${saved ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'
                }`}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex justify-end">
              <button
                ...
                className={`flex items-center px-4 py-2 rounded-lg font-medium transition-colors ${htmlCopied
